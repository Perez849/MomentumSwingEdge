"""
╔══════════════════════════════════════════════════════════════════════╗
║  MOMENTUM BREAKOUT ENGINE  v2.0  — ML Edition                        ║
║  Swing trading de alta convicción + ML Setup Filter                  ║
╠══════════════════════════════════════════════════════════════════════╣
║                                                                      ║
║  NUEVO EN v2.0:                                                      ║
║  ML Setup Filter — LightGBM walk-forward que aprende qué            ║
║  setups son de alta calidad a partir del historial IS del propio     ║
║  sistema. Filtra entradas OOS bloqueando breakouts de baja           ║
║  probabilidad antes de que abran posición.                           ║
║                                                                      ║
║  IMPACTO ESPERADO:                                                   ║
║  +5-12pp Win Rate · +0.3-0.8 PF · -25% frecuencia                  ║
║                                                                      ║
...
║  2. COMPRESIÓN: volatilidad realizada en percentil ≤ 35              ║
║  3. BREAKOUT: rompe máximo de la compresión con volumen ≥ 65         ║
║                                                                      ║
║  ML FILTER (nuevo):                                                  ║
║  LightGBM entrenado con trades IS → score 0-100 → umbral auto       ║
║  Solo operan setups con score ≥ umbral calibrado                     ║
║                                                                      ║
...
# ── ML Filter ─────────────────────────────────────────────────────
try:
    from ml_filter import MLSetupFilter, extract_features
    HAS_ML = True
except ImportError:
    HAS_ML = False
...
UNIVERSE = {
    ...
    # ── ANTI-SEQUÍA energía + oro + defensa ──────────────────────
...
HIGH_VOL_MOMENTUM = {
    "OXY", "DVN", "FANG", "MPC", "MRO", "AR", "HES", "CEG",
...
PANIC_VOL_HIGH_MOMENTUM = 92

TIER1_ASSETS = {
    "ZPDJ.DE","EXV1.DE","DFEN.DE","WTIC.DE",
    "XLE","XLI","XME","ITA","COPX","LIT",
    "GOOGL","AVGO","TDG","PANW","CRWD","VST","RCL",
    "G2X.DE","LMT","OXY","CEG",
}
TIER2_ASSETS = {
    "IS0E.DE","SLVR.DE","VVSM.DE","WTIF.DE","EMXC.DE",
...
TRAIL_PCT_1    = 0.05;  TRAIL_PCT_2    = 0.035; TRAIL_PCT_3    = 0.02
TRAIL_GAIN_1   = 0.40;  TRAIL_GAIN_2   = 0.55;  TRAIL_GAIN_3   = 0.75
...
PB_TOUCH_BUFFER   = 0.005; PB_VOL_MAX_RANK = 45
PB_TREND_MIN_BARS = 10;    PB_PANIC_MAX    = 80
PB_TP_R           = 2.5;   PB_MAX_HOLD     = 20
PB_SL_BUFFER      = 0.004; PB_MIN_RISK     = 0.008
PB_MAX_RISK       = 0.10;  PB_POSITION_SIZE = 0.10
...
# ════════════════════════════════════════════════════════════════════
# BACKTEST — con soporte ML Filter
# ════════════════════════════════════════════════════════════════════

def backtest(ticker, ind, ml_filter=None, record_features=False):
    """
    Backtest barra a barra.

    ml_filter:        instancia MLSetupFilter para filtrar entradas
    record_features:  True en IS para guardar setup_features en cada trade
    """
    n      = ind['n']
    trades = []
    diag   = {'panic': 0, 'no_trend': 0, 'no_comp': 0, 'no_bo': 0,
              'all3': 0, 'bad_levels': 0, 'ml_blocked': 0}

    in_trade    = False
    entry_price = sl = sl_initial = tp = peak = 0.0
    entry_i     = 0
    entry_date  = None
    setup_snap  = {}   # snapshot del setup para guardar en el trade
    ...
    for i in range(COMPRESSION_BARS + 252 + TREND_BARS, n):
        ...
        if in_trade:
            ...
            if reason:
                ...
                r_achieved = round((peak - entry_price) / (entry_price - sl_initial), 2) \
                             if (entry_price - sl_initial) > 0 else 0
                ...
                trade_dict = {
                    ...
                    'setup_features': extract_features(setup_snap) if (record_features and HAS_ML and setup_snap) else None,
                    'ml_score':       setup_snap.get('ml_score'),
                }
        ...
        # ── ML FILTER ────────────────────────────────────────────
        ml_score = None
        if ml_filter is not None and HAS_ML:
            snap_for_ml = {
                'ticker':       ticker,
                'vol_pct_now':  float(ind['vol_pct'][i])  if not np.isnan(ind['vol_pct'][i])  else 50.0,
                'vol_rank_now': float(ind['vol_rank'][i])  if not np.isnan(ind['vol_rank'][i]) else 50.0,
                'price':        float(ind['c'][i]),
            ...
            ok_ml, ml_score, _ = ml_filter.should_trade(snap_for_ml)
            if not ok_ml:
                diag['ml_blocked'] += 1
                continue
        ...
        # Guardar snapshot para features IS
        setup_snap = {
            'ticker':       ticker,
            'vol_pct_now':  float(ind['vol_pct'][i])  if not np.isnan(ind['vol_pct'][i])  else 50.0,
            'vol_rank_now': float(ind['vol_rank'][i])  if not np.isnan(ind['vol_rank'][i]) else 50.0,
...
def portfolio_simulate(trades, initial_capital=10000.0, position_size_pct=15.0):
    ...
    cagr  = float((equity[-1] / initial_capital) ** (1 / max(years, 0.1)) - 1) * 100 \
            if years > 0 else 0
...
def get_today_signal(ticker, ind, ml_filter=None):
    i = ind['n'] - 1
    ...
    if not panic and trend_ok and comp_ok and bo_ok:
        ...
        if entry_p and rr >= 1.5:
            ...
            if ml_filter is not None and HAS_ML:
                ...
                ok_ml, ml_score, _ = ml_filter.should_trade(snap)
                signal_type = 'FIRE' if ok_ml else 'FIRE_BLOCKED'
            else:
                signal_type = 'FIRE'
        else:
            signal_type = 'WEAK'
    ...
    sl_pct = tp_pct = None
    if signal_type in ('FIRE', 'FIRE_BLOCKED'):
        entry_p, sl_p, tp_p, rr = compute_levels(ind, i, comp_d, ticker)
        if entry_p and sl_p:
...
<title>Momentum Breakout Engine v2.0</title>
...
.sig.blocked{border-color:rgba(123,94,167,.25);opacity:.7}
...
.sig.blocked .sig-stripe{background:linear-gradient(90deg,var(--purple),transparent)}
...
.sig.blocked .sig-badge{background:rgba(123,94,167,.12);color:#b8a0e8;border:1px solid rgba(123,94,167,.25)}
...
.ml-score-bar{margin-top:.6rem;padding:.5rem .65rem;background:var(--s2);border-radius:8px;border:1px solid var(--b2)}
.ml-score-lbl{font-family:var(--mono);font-size:.48rem;text-transform:uppercase;letter-spacing:.09em;color:var(--t3);margin-bottom:.3rem}
.ml-bar-bg{background:var(--b1);border-radius:4px;height:6px;overflow:hidden}
.ml-bar-fill{height:100%;border-radius:4px;transition:width .5s}
...
.sp-blocked{background:rgba(123,94,167,.08);color:#a08cc8;border:1px solid rgba(123,94,167,.18)}
...
<header>
  <div class="brand">
    ...
    <div>
      ...
      <div class="brand-sub">v2.0 · ML Edition</div>
    </div>
  </div>
  <div class="hdr-meta">
    <div class="pulse"></div>
...
<div id="ml-section"></div>
<div id="fire-section" style="display:none">
  <div class="section-head">🔥 Señales de entrada — validadas por ML Filter</div>
  <div class="signals-grid" id="fire-grid"></div>
</div>
<div id="blocked-section" style="display:none">
  <div class="section-head">🧠 Bloqueadas por ML Filter — setup técnico válido, score insuficiente</div>
  <div class="signals-grid" id="blocked-grid"></div>
</div>
...
const fire_sigs    = Object.values(sigs).filter(s=>s.signal==='FIRE');
const blocked_sigs = Object.values(sigs).filter(s=>s.signal==='FIRE_BLOCKED');
const watch_sigs   = Object.values(sigs).filter(s=>s.signal==='WATCH');
const ml           = D.ml_filter_stats||{};

// Stats bar
const stats=[
  {lbl:'Señales hoy',val:fire_sigs.length,sub:'FIRE · validadas ML',accent:'var(--fire)'},
  {lbl:'Bloqueadas ML',val:blocked_sigs.length,sub:'Setup OK, score bajo',accent:'var(--purple)'},
  {lbl:'WR Out-of-Sample',val:oos?oos.wr+'%':'—',sub:`${oos?.n||0} trades OOS`,
    accent:oos&&oos.wr>=60?'var(--safe)':oos&&oos.wr>=50?'var(--watch)':'var(--fire)'},
  {lbl:'Profit Factor',val:oos?oos.pf:'—',sub:'Ganancias / Pérdidas',
    accent:oos&&oos.pf>=1.8?'var(--safe)':oos&&oos.pf>=1.3?'var(--watch)':'var(--blue)'},
  {lbl:'Sharpe OOS',val:oos?oos.sharpe:'—',sub:'Anualizado',
    accent:oos&&oos.sharpe>=1.0?'var(--safe)':'var(--blue)'},
  {lbl:'Max Drawdown',val:oos?oos.max_dd+'%':'—',sub:'Pico-valle',
    accent:oos&&oos.max_dd>-15?'var(--safe)':'var(--fire)'},
  {lbl:'ML Umbral',val:ml.threshold?ml.threshold+'%':'—',sub:'Score mínimo para operar',accent:'var(--purple)'},
  {lbl:'Trades/mes OOS',val:D.sigs_per_month||'—',sub:'Frecuencia filtrada',accent:'var(--blue)'},
];
$('stats-section').innerHTML=`
  <div class="section-head">Estado del sistema — v2.0 ML Edition</div>
...
// ML stats card
if(ml&&ml.n_trades){
  const wrImp=ml.wr_improvement||0; const pfImp=ml.pf_improvement||0;
  const wrCol=wrImp>3?'var(--safe)':wrImp>0?'var(--watch)':'var(--fire)';
  const pfCol=pfImp>0.2?'var(--safe)':pfImp>0?'var(--watch)':'var(--fire)';
  const imp=Object.entries(ml.feature_importance||{}).sort((a,b)=>b[1]-a[1]).slice(0,6);
  const maxImp=imp[0]?imp[0][1]:1;
  $('ml-section').innerHTML=`
    <div class="section-head">🧠 ML Setup Filter — resultados IS</div>
    <div style="background:var(--s1);border:1px solid rgba(123,94,167,.3);border-radius:var(--r);padding:1.4rem;display:grid;grid-template-columns:1fr 1fr;gap:1.5rem">
      <div>
        <div style="font-family:var(--mono);font-size:.58rem;color:var(--t3);margin-bottom:.8rem;text-transform:uppercase;letter-spacing:.1em">Impacto del filtro (IS)</div>
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:.5rem;margin-bottom:1rem">
          ${[['Base WR',ml.base_wr+'%'],['Filtrado WR',ml.filtered_wr+'%'],['Mejora',`<span style="color:${wrCol}">${wrImp>0?'+':''}${wrImp}pp</span>`]].map(([l,v])=>`
          <div style="background:var(--s2);border-radius:8px;padding:.6rem .8rem">
            <div style="font-family:var(--mono);font-size:.48rem;color:var(--t3);margin-bottom:.3rem">${l}</div>
            <div style="font-family:var(--mono);font-size:.9rem">${v}</div>
          </div>`).join('')}
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:.5rem">
          ${[['Base PF',ml.base_pf],['Filtrado PF',ml.filtered_pf],['Mejora',`<span style="color:${pfCol}">${pfImp>0?'+':''}${pfImp}</span>`]].map(([l,v])=>`
          <div style="background:var(--s2);border-radius:8px;padding:.6rem .8rem">
            <div style="font-family:var(--mono);font-size:.48rem;color:var(--t3);margin-bottom:.3rem">${l}</div>
            <div style="font-family:var(--mono);font-size:.9rem">${v}</div>
          </div>`).join('')}
        </div>
        <div style="margin-top:.8rem;font-family:var(--mono);font-size:.58rem;color:var(--t3)">
          Entrenado con ${ml.n_trades} trades · Umbral auto: ${ml.threshold}% · 
          Pasaron: ${ml.n_passed} (${Math.round(ml.n_passed/ml.n_trades*100)}%) · 
          Bloqueados: ${ml.n_blocked}
        </div>
      </div>
      <div>
        <div style="font-family:var(--mono);font-size:.58rem;color:var(--t3);margin-bottom:.8rem;text-transform:uppercase;letter-spacing:.1em">Feature importance</div>
        ${imp.map(([f,v])=>`
        <div style="margin-bottom:.5rem">
          <div style="display:flex;justify-content:space-between;margin-bottom:.2rem">
            <span style="font-family:var(--mono);font-size:.6rem;color:var(--t2)">${f}</span>
            <span style="font-family:var(--mono);font-size:.58rem;color:var(--t3)">${v.toFixed(4)}</span>
          </div>
          <div style="background:var(--b1);border-radius:3px;height:4px;overflow:hidden">
            <div style="width:${v/maxImp*100}%;background:linear-gradient(90deg,#7b5ea7,#b8a0e8);height:100%;border-radius:3px"></div>
          </div>
        </div>`).join('')}
      </div>
    </div>`;
...
// Signal cards
function mlScoreHTML(score){
  if(score==null) return '';
  const pct=Math.round(score*100);
  const col=pct>=65?'var(--safe)':pct>=50?'var(--watch)':'var(--fire)';
  return`<div class="ml-score-bar">
    <div class="ml-score-lbl">ML Score</div>
    <div style="display:flex;align-items:center;gap:.6rem">
      <div class="ml-bar-bg" style="flex:1">
        <div class="ml-bar-fill" style="width:${pct}%;background:${col}"></div>
      </div>
      <span style="font-family:var(--mono);font-size:.75rem;color:${col};font-weight:600">${pct}%</span>
    </div>
  </div>`;
}

function buildCard(s,isBlocked=false){
  const isFire=s.signal==='FIRE';
  const cls=isBlocked?'blocked':isFire?'fire':'watch';
  const badge=isBlocked?'🧠 Bloqueado por ML':isFire?'🔥 Entrar ahora':'👁 Vigilar breakout';
  return`<div class="sig ${cls}" onclick="openDrawer('${s.ticker}')">
    <div class="sig-stripe"></div>
    <span class="sig-badge">${badge}</span>
    <div class="sig-ticker">${s.ticker}</div>
    <div class="sig-name">${s.name||''}</div>
    ${(isFire||isBlocked)&&s.entry_price?`
    <div class="levels">
...
if(fire_sigs.length){$('fire-section').style.display='block';$('fire-grid').innerHTML=fire_sigs.map(s=>buildCard(s,false)).join('');}
if(blocked_sigs.length){$('blocked-section').style.display='block';$('blocked-grid').innerHTML=blocked_sigs.map(s=>buildCard(s,true)).join('');}
if(watch_sigs.length){$('watch-section').style.display='block';$('watch-grid').innerHTML=watch_sigs.map(buildCard).join('');}
...
if(is_&&oos){
  const rows=[
    ...
    ['Eficiencia pico','avg_efficiency','%'],['Racha pérdidas','max_loss_streak',''],
  ];
  const verdict=oos.wr>=60&&oos.pf>=1.5&&oos.sharpe>=0.8?
    {t:'✅ SISTEMA VÁLIDO + ML FILTER',c:'var(--safe)',d:'Edge real con filtro ML — calidad sobre cantidad.'}:
    oos.wr>=55&&oos.pf>=1.2?
    {t:'⚡ SISTEMA SÓLIDO',c:'var(--watch)',d:'Edge demostrado. Continuar con tamaño normal.'}:
    oos.wr>=50&&oos.pf>=1.1?
    {t:'⚡ SISTEMA MARGINAL',c:'var(--watch)',d:'Edge débil. Operar con tamaño reducido.'}:
    {t:'✗ SIN EDGE ESTADÍSTICO',c:'var(--fire)',d:'Revisar parámetros o universo.'};
  ...
  $('bt-section').innerHTML=`
    ...
    <div style="background:var(--s1);border:1px solid ${verdict.c}33;border-left:4px solid ${verdict.c};border-radius:var(--r);padding:1.2rem 1.4rem;margin-bottom:1rem">
      ...
      <div style="font-size:.72rem;color:var(--t2)">${verdict.d}${D.benchmark?' · SPY mismo período: '+(D.benchmark.total_pct>=0?'+':'')+D.benchmark.total_pct+'%':''}</div>
    </div>
    <div class="bt-grid">
      <div class="bt-card">
...
if(port&&port.equity_curve&&port.equity_curve.length>1){
  ...
  if(D.benchmark&&D.benchmark.equity_curve&&D.benchmark.daily_dates){
    const spyMap={};
    D.benchmark.daily_dates.forEach((d,i)=>spyMap[d]=D.benchmark.equity_curve[i]);
    let last=100;
    spyCurve=portDates.map(d=>{if(spyMap[d]!=null)last=spyMap[d];return last;});
  }
  const hasSpy=spyCurve.length===nDays;
  $('eq-section').innerHTML=`
    <div class="section-head">Curva de equity — Portfolio OOS vs SPY</div>
    <div class="chart-card">
      <div class="chart-title" style="display:flex;gap:1.4rem;align-items:center;flex-wrap:wrap">
        <div style="display:flex;align-items:center;gap:.5rem">
          <div style="width:24px;height:3px;background:#06d6a0;border-radius:2px"></div>
          <span style="font-size:.62rem;color:var(--t2)">Portfolio ML-Filtered (${port.total_pct>=0?'+':''}${port.total_pct}% · CAGR ${port.cagr>=0?'+':''}${port.cagr}%)</span>
        </div>
        ${hasSpy?`<div style="display:flex;align-items:center;gap:.5rem">
          <div style="width:24px;height:2px;background:#4cc9f0;border-radius:2px;opacity:.7"></div>
          <span style="font-size:.62rem;color:var(--t2)">SPY (${D.benchmark.total_pct>=0?'+':''}${D.benchmark.total_pct}%)</span>
        </div>`:''}
      </div>
  ...
  setTimeout(()=>{
    ...
    const allV=[...eq,...(hasSpy?spyCurve:[]),100];
    const mn=Math.min(...allV)*.995,mx=Math.max(...allV)*1.005;
    ...
    for(let g=0;g<=5;g++){
      ...
      ctx.fillStyle='rgba(77,106,153,.85)';ctx.font='9px Syne Mono,monospace';ctx.textAlign='right';
      ctx.fillText(v>=100?'+'+((v-100).toFixed(0))+'%':'-'+((100-v).toFixed(0))+'%',pad.l-4,y+3);
    }
    ctx.textAlign='left';
    const y100=yp(100);
...
const sorted=Object.values(sigs).sort((a,b)=>{
  const order={FIRE:0,FIRE_BLOCKED:1,WATCH:2,TREND:3,NONE:4,PANIC:5};
  return (order[a.signal]??6)-(order[b.signal]??6);
});
$('radar-body').innerHTML=sorted.map(s=>{
  const dotColor=ok=>ok?'var(--safe)':'rgba(255,77,109,.35)';
  const spMap={FIRE:'sp-fire',FIRE_BLOCKED:'sp-blocked',WATCH:'sp-watch',TREND:'sp-trend',PANIC:'sp-panic',NONE:'sp-none'};
  const lbMap={FIRE:'🔥 FIRE',FIRE_BLOCKED:'🧠 Bloqueado',WATCH:'👁 Watch',TREND:'→ Trend',PANIC:'⚡ Pánico',NONE:'—'};
  const spClass=spMap[s.signal]||'sp-none';
  const spLabel=lbMap[s.signal]||'—';
  const rr=s.rr_ratio;
  const mlPct=s.ml_score!=null?Math.round(s.ml_score*100):null;
  const mlCol=mlPct>=65?'var(--safe)':mlPct>=50?'var(--watch)':'var(--fire)';
  return`<tr onclick="openDrawer('${s.ticker}')">
    ...
    <td>${mlPct!=null?`<span style="font-family:var(--mono);font-size:.7rem;color:${mlCol};font-weight:600">${mlPct}%</span>`:'<span style="color:var(--t4)">—</span>'}</td>
...
if(oos_trades.length){
  $('history-section').innerHTML=`
    <div class="section-head">Histórico trades OOS — ${oos_trades.length} operaciones (ML-Filtered)</div>
    <div class="trades-tbl-wrap">
      <table class="tbl">
        <thead><tr>
          <th>Ticker</th><th>ML%</th><th>Entrada</th><th>Salida</th>
          <th>$ Entrada</th><th>$ Salida</th>
          <th>P&L</th><th>Pico</th><th>R máx</th><th>Días</th><th>Motivo</th>
        </tr></thead>
        <tbody>${oos_trades.map(t=>{
          const win=t.pnl>=0;
          const sc=t.ml_score!=null?Math.round(t.ml_score*100):null;
          const scCol=sc>=65?'var(--safe)':sc>=50?'var(--watch)':'var(--t3)';
          return`<tr>
            <td style="font-weight:600;font-family:var(--mono)">${t.ticker}</td>
            <td>${sc!=null?`<span style="font-family:var(--mono);font-size:.68rem;color:${scCol}">${sc}%</span>`:'—'}</td>
            ...
            <td><span style="font-family:var(--mono);font-size:.6rem;font-weight:600;padding:.15rem .4rem;border-radius:5px;${
              t.reason==='TP'?'background:rgba(6,214,160,.12);color:var(--safe)':
              t.reason==='SL'?'background:rgba(255,77,109,.1);color:var(--fire)':
              t.reason==='C'?'background:rgba(255,214,10,.1);color:var(--watch)':
...
function openDrawer(ticker){
  ...
  $('drawer-content').innerHTML=`
    ...
    ${mlPct!=null?`
    <div style="background:var(--s2);border-radius:8px;padding:.7rem 1rem;margin-bottom:1rem;border:1px solid rgba(123,94,167,.2)">
      <div style="font-family:var(--mono);font-size:.5rem;text-transform:uppercase;letter-spacing:.1em;color:var(--t3);margin-bottom:.4rem">ML Score</div>
      <div style="display:flex;align-items:center;gap:.8rem">
        <div style="flex:1;background:var(--b1);border-radius:4px;height:8px;overflow:hidden">
          <div style="width:${mlPct}%;background:${mlCol};height:100%;border-radius:4px"></div>
        </div>
        <span style="font-family:var(--mono);font-size:1.1rem;color:${mlCol};font-weight:700">${mlPct}%</span>
      </div>
      ${s.signal==='FIRE_BLOCKED'?'<div style="font-size:.6rem;color:var(--fire);margin-top:.3rem">⚠ Bloqueado — score bajo umbral</div>':''}
    </div>`:''}
    <div style="display:flex;gap:.5rem;flex-wrap:wrap;margin-bottom:1.2rem">
      <span class="cond ${s.trend?'ok':'no'}">${s.trend?'✓':'✗'} Tendencia</span>
      <span class="cond ${s.compressed?'ok':'no'}">${s.compressed?'✓':'✗'} Compresión</span>
      <span class="cond ${s.breakout?'ok':'no'}">${s.breakout?'✓':'✗'} Breakout</span>
      ${s.panic?'<span class="cond no">⚡ PÁNICO</span>':''}
    </div>
    ${(s.signal==='FIRE'||s.signal==='FIRE_BLOCKED')&&s.entry_price?`
    ...
    <div class="chart-card">
      <div class="chart-title">Percentil volatilidad histórica</div>
  ...
  setTimeout(()=>{
    ...
    drawLineChart('d-price',[
      ...
      {data:hist.map(x=>x.ema50),c:'#7b5ea7',w:1,d:[5,5]},
       ...
       refs:s.stop_loss?[{v:s.stop_loss,c:'rgba(255,77,109,.4)',l:'SL'},{v:s.take_profit,c:'rgba(6,214,160,.4)',l:'TP'}]:[]});
    drawLineChart('d-vol',[
...
def main():
    print(f"\n{BOLD}{'═'*65}{RST}")
    print(f"{BOLD}{C}  ◈  MOMENTUM BREAKOUT ENGINE  v2.0 — ML Edition{RST}")
    print(f"{DIM}  ML Setup Filter · LightGBM walk-forward · auto-calibrado{RST}")
    print(f"{BOLD}{'═'*65}{RST}\n")

    print(f"  {C}↓ Descargando histórico ({len(UNIVERSE)} activos)...{RST}\n")
    ...
    # ── Fase 1: backtest IS (sin ML filter) — recoger datos ─────────
    print(f"  {DIM}Fase 1/3 — Backtest IS (entrenamiento ML)...{RST}\n")

    indicator_cache = {}  # cache para no descargar dos veces
    ...
    for ticker, name in UNIVERSE.items():
        ...
        try:
            ...
            if len(df) < 600:
                print(f"{Y}⚠ {len(df)} barras{RST}")
                continue
            print(f"{G}✓{RST} {len(df)}b", end="  ", flush=True)
        ...
        # IS: guardar features para entrenar ML
        trades_is, _ = backtest(ticker, ind_is,
                                ml_filter=None,
                                record_features=True)
        all_is_trades.extend(trades_is)

        # Guardar indicadores full para fase 2
        indicator_cache[ticker] = (df, ind_full, oos_start_str)

        n_is = len(trades_is)
        m_is = compute_metrics(trades_is)
        print(f"IS:{n_is:>3}t WR={m_is['wr'] if m_is else 0:.0f}%")

    # ── Fase 2: entrenar ML Filter ───────────────────────────────────
    ml_filter = None
    if HAS_ML:
        print(f"\n  {BOLD}{C}Fase 2/3 — Entrenando ML Filter con {len(all_is_trades)} trades IS...{RST}")
        ml_filter = MLSetupFilter()
        ml_filter.train(all_is_trades)
        ml_filter.print_stats()
    else:
        print(f"\n  {Y}⚠ LightGBM no disponible — ejecutando sin ML Filter{RST}\n")

    # ── Fase 3: backtest OOS con ML Filter ───────────────────────────
    print(f"  {DIM}Fase 3/3 — Backtest OOS con ML Filter...{RST}\n")

    for ticker, (df, ind_full, oos_start_str) in indicator_cache.items():
        ...
        n_ml  = diag_full.get('ml_blocked', 0)
        col   = G if (m_oos and m_oos['wr'] >= 55) else (Y if (m_oos and m_oos['wr'] >= 45) else R)
        print(f"  {ticker:<12} OOS:{n_oos:>3}t "
              f"{col}WR={m_oos['wr'] if m_oos else 0:.0f}%{RST} "
              f"PF={m_oos['pf'] if m_oos else 0:.2f} "
              f"{DIM}ML↓{n_ml}{RST}")

        # Señales de hoy
        sig = get_today_signal(ticker, ind_full, ml_filter=ml_filter)
        if sig:
    ...
    if m_is_global and port_is:
        ...
        m_is_global['cagr']      = port_is.get('cagr', 0)
    if m_oos_global and port_oos:
        ...
        m_oos_global['cagr']      = port_oos.get('cagr', 0)
    ...
    try:
        if oos_dates:
            spy = yf.download("SPY", start=oos_dates[0],
                              end=max(t['exit_date'] for t in all_oos_trades),
                              auto_adjust=True, progress=False)
            if isinstance(spy.columns, pd.MultiIndex):
                spy.columns = spy.columns.get_level_values(0)
            if len(spy) >= 2:
                spy_c   = spy['Close'].squeeze()
                spy_m   = np.cumprod(1 + spy_c.pct_change().fillna(0).values)
                spy_run = np.maximum.accumulate(spy_m)
                benchmark = {
                    'total_pct':    round(float((spy_m[-1]-1)*100), 1),
                    'max_dd':       round(float(((spy_m/spy_run)-1).min()*100), 1),
                    'daily_dates':  [d.strftime('%Y-%m-%d') for d in spy_c.index],
                    'equity_curve': [round(float(v*100), 2) for v in spy_m],
                }
    except Exception:
        pass
    ...
    def _row(lbl, mi, mo, key, fmt="{:.1f}", suf=""):
        vi = mi[key] if mi else None
        vo = mo[key] if mo else None
        better_fn = lambda a, b: a > b if key not in ('max_dd','avg_loss','max_loss_streak') else a < b
        col = (G if (vi is not None and vo is not None and better_fn(vo, vi))
               else R if (vi is not None and vo is not None) else "")
    ...
    _row("Sharpe (portfolio)",m_is_global, m_oos_global, 'sharpe',     "{:.2f}")
    ...
    if benchmark:
        print(f"\n  SPY mismo período OOS: "
              f"{'+' if benchmark['total_pct']>=0 else ''}{benchmark['total_pct']:.1f}%")

    fire_today    = [tk for tk, s in all_signals.items() if s['signal'] == 'FIRE']
    blocked_today = [tk for tk, s in all_signals.items() if s['signal'] == 'FIRE_BLOCKED']
    watch_today   = [tk for tk, s in all_signals.items() if s['signal'] == 'WATCH']

    if fire_today:
        print(f"\n  {M}{BOLD}🔥 SEÑALES HOY (ML validadas): {', '.join(fire_today)}{RST}")
    if blocked_today:
        print(f"  {DIM}🧠 Bloqueadas ML: {', '.join(blocked_today)}{RST}")
    if watch_today:
    ...
    if m_oos_global:
        ...
        if wr >= 60 and pf >= 1.5 and sh >= 0.8:
            print(f"\n  {G}{BOLD}✅ SISTEMA PODEROSO — ML Filter funcionando{RST}")
            ...
            print(f"\n  {R}{BOLD}✗ SIN EDGE — revisar condiciones{RST}")
    ...
    dashboard_data = _san({
        ...
        "ml_filter_stats":  ml_filter.stats if (ml_filter and ml_filter.trained) else {},
        "params": {
