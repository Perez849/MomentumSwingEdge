...
║  Swing trading adaptativo — momentum + pullback + régimen bajista    ║
╠══════════════════════════════════════════════════════════════════════╣
║                                                                      ║
║  CAMBIOS CLAVE vs v1.1:                                              ║
║                                                                      ║
║  1. TRAILING CHANDELIER ADAPTATIVO (ATR):                            ║
║     - peak - N×ATR dinámico (se adapta al activo)                    ║
║     - Floor de ganancia mínima en cada fase                          ║
║     - Evita devolver +8% → -6%                                      ║
║                                                                      ║
║  2. PULLBACK EN TENDENCIA REACTIVADO:                                ║
║     - Opera en Tier1+Tier2 cuando precio toca EMA20/50               ║
║     - Elimina sequías: genera señales cuando momentum descansa       ║
║                                                                      ║
║  3. RÉGIMEN BAJISTA → ACTIVOS INVERSOS:                              ║
║     - SPY < EMA200 + pendiente EMA50 negativa → bear mode           ║
║     - Activa DXS3.DE (S&P inverse), VIXL.DE etc.                    ║
║                                                                      ║
║  4. TIME STOP INTELIGENTE:                                           ║
║     - Si no alcanza 0.3R en 10 días → salir                         ║
║     - Libera capital de trades zombis                                ║
║                                                                      ║
║  5. BREAK-EVEN MÁS AGRESIVO: 0.4R (antes 0.6R)                     ║
║                                                                      ║
...
INVERSE_ASSETS = {"VIXL.DE", "DXS3.DE"}
DEFENSIVE_ASSETS = {"IS0E.DE", "NEM", "AEM", "PAAS", "WTIC.DE", "XLV", "NLR", "IUSR.DE"}
...
# ── Gestión del trade v2.0 — Chandelier Exit ─────────────────────
TP_R_MULTIPLE       = 5.0
SL_BUFFER_PCT       = 0.5
MAX_HOLD_DAYS       = 35
BREAKEVEN_AT_R      = 0.4    # ▼ 0.6→0.4: BE más temprano

# Chandelier trailing por fases (peak - N×ATR)
CHAN_ACT_1_R  = 1.0;  CHAN_ATR_1 = 3.0;  CHAN_FLOOR_1 = 0.30
CHAN_ACT_2_R  = 2.0;  CHAN_ATR_2 = 2.0;  CHAN_FLOOR_2 = 0.50
CHAN_ACT_3_R  = 3.0;  CHAN_ATR_3 = 1.5;  CHAN_FLOOR_3 = 0.70

# Time stop: sin progreso → salir
PROGRESS_DAYS   = 10
MIN_PROGRESS_R  = 0.3
...
PB_EMA_FAST = 20; PB_EMA_MID = 50; PB_TOUCH_BUFFER = 0.005
PB_VOL_MAX_RANK = 45; PB_TREND_MIN_BARS = 10; PB_PANIC_MAX = 80
PB_TP_R = 2.5; PB_MAX_HOLD = 20; PB_SL_BUFFER = 0.004
PB_MIN_RISK = 0.008; PB_MAX_RISK = 0.10; PB_POSITION_SIZE = 0.10
PB_CHAN_ATR_1 = 2.5; PB_CHAN_ATR_2 = 1.8
PB_CHAN_FLOOR_1 = 0.35; PB_CHAN_FLOOR_2 = 0.60

BEAR_REGIME_LOOKBACK = 5
...
def compute_indicators(df):
    ...
    def percentile_roll(series, window=252):
        result = np.full(len(series), np.nan)
        arr = series.values
        for i in range(window, len(arr)):
            if np.isnan(arr[i]): continue
            hist = arr[i-window:i]; hist = hist[~np.isnan(hist)]
            if len(hist) < 50: continue
            result[i] = float(np.sum(hist <= arr[i]) / len(hist) * 100)
        return result

    vol_pct = percentile_roll(vol20)
    ...
    vol_rank_arr = percentile_roll(v)

    delta = c.diff(); gain = delta.clip(lower=0); loss_s = (-delta).clip(lower=0)
    avg_gain = gain.ewm(span=3, adjust=False).mean()
    avg_loss = loss_s.ewm(span=3, adjust=False).mean()
    rs = avg_gain / avg_loss.replace(0, np.nan)
    rsi3 = 100 - (100 / (1 + rs))

    roc20 = c.pct_change(20) * 100
    ema50_slope = (ema50 - ema50.shift(BEAR_REGIME_LOOKBACK)) / ema50.shift(BEAR_REGIME_LOOKBACK) * 100

    return {
        'c': c.values.astype(float), 'h': h.values.astype(float),
        'lo': lo.values.astype(float), 'op': op.values.astype(float),
        'v': v.values.astype(float), 'tr': tr.values.astype(float),
        'atr14': atr14.values.astype(float), 'vol20': vol20.values.astype(float),
        'vol_pct': vol_pct, 'vol_rank': vol_rank_arr,
        ...
        'rsi3': rsi3.values.astype(float), 'roc20': roc20.values.astype(float),
        'ema50_slope': ema50_slope.values.astype(float),
        'dates': df.index, 'n': len(c),
    }
...
def detect_market_regime():
    ...
    try:
        ...
        if len(spy) < 220:
            _spy_regime_cache = {'regime': 'NEUTRAL', 'dates': {}}
            return _spy_regime_cache
        c = spy['Close'].squeeze().astype(float)
        ema200 = c.ewm(span=200, adjust=False).mean()
        ema50  = c.ewm(span=50, adjust=False).mean()
        regimes = {}
        for i in range(200, len(c)):
            date_str = str(spy.index[i])[:10]
            price = float(c.iloc[i]); e200 = float(ema200.iloc[i])
            e50 = float(ema50.iloc[i]); e50_prev = float(ema50.iloc[max(0, i-BEAR_REGIME_LOOKBACK)])
            if price < e200 and e50 < e50_prev:
                regimes[date_str] = 'BEAR'
            elif price < e200 or e50 < e50_prev:
                regimes[date_str] = 'CAUTION'
            else:
                regimes[date_str] = 'BULL'
        _spy_regime_cache = {'regime': regimes.get(list(regimes.keys())[-1], 'NEUTRAL'), 'dates': regimes}
    except Exception:
        _spy_regime_cache = {'regime': 'NEUTRAL', 'dates': {}}
    return _spy_regime_cache
...
def check_trend(ind, i, ticker=''):
    ...
    price_now = _v(ind, 'c', i); price_then = _v(ind, 'c', i - TREND_BARS)
    ema50_now = _v(ind, 'ema50', i); atr_now = _v(ind, 'atr14', i)
    ...
    es_especial = ticker in INVERSE_ASSETS or ticker in DEFENSIVE_ASSETS
    ema200_now = _v(ind, 'ema200', i)
    above_ema200 = es_especial or (price_now > ema200_now and ema200_now > 0)
    ...
    return ok, {
        ...
        'above_ema50': above_ema50, 'ema50': round(ema50_now, 2),
    }
...
def check_compression(ind, i):
    ...
    return compressed, {
        'vol_percentile': round(vp, 1), 'comp_high': round(comp_high, 4),
        'comp_low': round(comp_low, 4), 'comp_bars': COMPRESSION_BARS,
    }

def check_breakout(ind, i):
    if i < COMPRESSION_BARS + 252: return False, {}
    high = _v(ind, 'h', i); close = _v(ind, 'c', i)
    vr = ind['vol_rank'][i]
    if np.isnan(vr): return False, {}
    prev_high = float(np.max(ind['h'][i - COMPRESSION_BARS:i]))
    atr_now = _v(ind, 'atr14', i); min_breakout = prev_high + 0.20 * atr_now
    ...
    return ok, {
        'prev_high': round(prev_high, 4), 'price': round(close, 4),
        'vol_rank': round(vr, 1),
...
def compute_levels(ind, i, comp_detail, ticker=""):
    ...
    if risk / entry < 0.010: return None, None, None, None
    tp = entry + risk * TP_R_MULTIPLE
    rr = round((tp - entry) / risk, 2)
    return round(entry, 4), round(sl, 4), round(tp, 4), rr

# ════════════════════════════════════════════════════════════════════
# TRAILING CHANDELIER v2.0
# ════════════════════════════════════════════════════════════════════

def compute_chandelier_sl(peak, entry_price, sl_current, sl_initial, atr_today,
                          pnl_r_peak, strategy='MOM'):
    """
    Chandelier Exit adaptativo. SL = peak - N×ATR con floor de ganancia.
    N se ajusta por fase. Floor garantiza captura mínima.
    SL solo sube (ratchet).
    """
    sl = sl_current
    risk = entry_price - sl_initial
    if risk <= 0 or atr_today <= 0: return sl
    gain = peak - entry_price

    # Break-even a 0.4R
    if pnl_r_peak >= BREAKEVEN_AT_R and sl < entry_price:
        sl = max(sl, entry_price * 1.001)

    if strategy == 'MOM':
        if pnl_r_peak >= CHAN_ACT_1_R and gain > 0:
            sl = max(sl, peak - CHAN_ATR_1 * atr_today, entry_price + CHAN_FLOOR_1 * gain)
        if pnl_r_peak >= CHAN_ACT_2_R and gain > 0:
            sl = max(sl, peak - CHAN_ATR_2 * atr_today, entry_price + CHAN_FLOOR_2 * gain)
        if pnl_r_peak >= CHAN_ACT_3_R and gain > 0:
            sl = max(sl, peak - CHAN_ATR_3 * atr_today, entry_price + CHAN_FLOOR_3 * gain)
        if pnl_r_peak >= 4.0 and gain > 0:
            sl = max(sl, peak - 1.0 * atr_today, entry_price + 0.85 * gain)
    elif strategy == 'PB':
        if pnl_r_peak >= 0.8 and gain > 0:
            sl = max(sl, peak - PB_CHAN_ATR_1 * atr_today, entry_price + PB_CHAN_FLOOR_1 * gain)
        if pnl_r_peak >= 1.5 and gain > 0:
            sl = max(sl, peak - PB_CHAN_ATR_2 * atr_today, entry_price + PB_CHAN_FLOOR_2 * gain)
    return sl

# ════════════════════════════════════════════════════════════════════
# BACKTEST MOMENTUM v2.0
# ════════════════════════════════════════════════════════════════════

def backtest(ticker, ind, market_regimes=None):
    n = ind['n']; trades = []
    diag = {'panic': 0, 'no_trend': 0, 'no_comp': 0, 'no_bo': 0,
            'all3': 0, 'bad_levels': 0, 'time_stop': 0, 'regime_skip': 0}
    in_trade = False
    entry_price = sl = sl_initial = tp = peak = 0.0
    entry_i = 0; entry_date = None
    is_inverse = ticker in INVERSE_ASSETS
    ...
    for i in range(COMPRESSION_BARS + 252 + TREND_BARS, n):
        ...
        if in_trade:
            high_today = _v(ind, 'h', i); low_today = _v(ind, 'lo', i)
            atr_today = _v(ind, 'atr14', i); held = i - entry_i
            risk = entry_price - sl_initial
            if risk <= 0 or atr_today <= 0: in_trade = False; continue

            pnl_r_peak = (peak - entry_price) / risk

            # Trailing Chandelier v2.0
            sl = compute_chandelier_sl(peak, entry_price, sl, sl_initial, atr_today, pnl_r_peak, 'MOM')
            peak = max(peak, high_today)

            # Time stop: sin progreso → salir
            reason = None
            if held >= PROGRESS_DAYS:
                peak_r = (peak - entry_price) / risk
                if peak_r < MIN_PROGRESS_R:
                    reason = 'P'; diag['time_stop'] += 1
            ...
            if reason:
                ...
                trades.append({
                    'ticker': ticker, 'entry_date': str(entry_date)[:10],
                    ...
                    'r_achieved': r_achieved, 'strategy': 'MOM',
                })
                in_trade = False
            continue

        # ── Nueva entrada ────────────────────────────────────────────
        if market_regimes and is_inverse:
            date_str = str(ind['dates'][i])[:10]
            regime = market_regimes.get(date_str, 'BULL')
            if regime not in ('BEAR', 'CAUTION'):
                diag['regime_skip'] += 1; continue

        if not is_inverse and check_panic(ind, i):
            diag['panic'] += 1; continue

        trend_ok, trend_d = check_trend(ind, i, ticker)
        if not trend_ok: diag['no_trend'] += 1; continue

        comp_ok, comp_d = check_compression(ind, i)
        if not comp_ok: diag['no_comp'] += 1; continue

        bo_ok, bo_d = check_breakout(ind, i)
        if not bo_ok: diag['no_bo'] += 1; continue

        diag['all3'] += 1
        entry_p, sl_p, tp_p, rr = compute_levels(ind, i, comp_d, ticker)
        if entry_p is None or rr < 1.5: diag['bad_levels'] += 1; continue

        in_trade = True; entry_price = entry_p; sl = sl_p
        sl_initial = sl_p; tp = tp_p; peak = _v(ind, 'h', i)
        entry_i = i; entry_date = ind['dates'][i]

    return trades, diag

# ════════════════════════════════════════════════════════════════════
# PULLBACK EN TENDENCIA v2.0
# ════════════════════════════════════════════════════════════════════

def backtest_pb(ticker, ind, momentum_open_dates=None):
    if ticker not in TIER1_ASSETS and ticker not in TIER2_ASSETS:
        return [], {}
    if momentum_open_dates is None: momentum_open_dates = set()
    ...
    for i in range(min_bars, n):
        ...
        if in_trade:
            ...
            atr_today = _v(ind, 'atr14', i); held = i - entry_i
            risk = entry_price - sl_initial
            if risk <= 0: in_trade = False; continue
            pnl_r_peak = (peak - entry_price) / risk

            # Trailing Chandelier para pullbacks
            sl = compute_chandelier_sl(peak, entry_price, sl, sl_initial, atr_today, pnl_r_peak, 'PB')
            peak = max(peak, high_today)
            ...
            if reason:
                ...
                trades.append({
                    ...
                    'pnl': round(pnl_net, 3), 'days': held, 'reason': reason,
                    'peak_pnl': round((peak - entry_price) / entry_price * 100, 2),
                    'r_achieved': r_achieved, 'strategy': 'PB',
                })
                in_trade = False
            continue

        # ── Nueva entrada pullback ───────────────────────────────────
        date_str = str(ind['dates'][i])[:10]
        if (ticker, date_str) in momentum_open_dates:
            diag['momentum_open'] += 1; continue

        vp = ind['vol_pct'][i]
        if not np.isnan(vp) and vp >= PB_PANIC_MAX:
            diag['panic'] += 1; continue

        ema20_now = _v(ind, 'ema20', i); ema50_now = _v(ind, 'ema50', i)
        ema200_now = _v(ind, 'ema200', i)
        if ema20_now <= 0 or ema50_now <= 0 or ema200_now <= 0:
            diag['no_trend'] += 1; continue
        ...
        touch_ema20 = touch_ema50 = False
        for j in range(max(0, i-3), i+1):
            lo_j = _v(ind, 'lo', j); c_j = _v(ind, 'c', j)
            e20_j = _v(ind, 'ema20', j); e50_j = _v(ind, 'ema50', j)
            if e20_j > 0 and lo_j <= e20_j * (1 + PB_TOUCH_BUFFER) and c_j >= e20_j:
                touch_ema20 = True
            if e50_j > 0 and ema200_now > 0:
                if (e50_j - ema200_now) / ema200_now >= 0.05 and lo_j <= e50_j * (1 + PB_TOUCH_BUFFER) and c_j >= e50_j:
                    touch_ema50 = True

        if not (touch_ema20 or touch_ema50):
            diag['no_touch'] += 1; continue
        if touch_ema20 and price_now < ema20_now:
            diag['no_pullback'] += 1; continue
        ...
        if risk <= 0 or risk / entry_p < PB_MIN_RISK:
            diag['bad_levels'] += 1; continue
        if risk / entry_p > PB_MAX_RISK:
            diag['bad_levels'] += 1; continue

        tp_p = entry_p + risk * PB_TP_R
        in_trade = True; entry_price = entry_p; sl = sl_p; sl_initial = sl_p
        tp = tp_p; peak = _v(ind, 'h', i); entry_i = i; entry_date = ind['dates'][i]
...
def compute_metrics(trades):
    ...
    wins = pnls[pnls > 0]; loss = pnls[pnls <= 0]; n = len(pnls)
    wr = len(wins) / n * 100
    ...
    by_reason = {}
    for t in trades: r = t.get('reason', '?'); by_reason[r] = by_reason.get(r, 0) + 1
    ...
    efficiencies = []
    for t in trades:
        peak_p = t.get('peak_pnl', 0); pnl_p = t.get('pnl', 0)
        if peak_p > 0.5: efficiencies.append(pnl_p / peak_p * 100)
    ...
    return {
        ...
        'expectancy': round(expectancy, 3), 'avg_days': round(avg_days, 1),
        'max_loss_streak': max_streak, 'avg_efficiency': avg_efficiency,
        'by_reason': by_reason,
...
def portfolio_simulate(trades, initial_capital=10000.0, position_size_pct=15.0):
    ...
    open_trades = []
    for t in trades_s:
        ei = date_idx.get(t['entry_date']); xi = date_idx.get(t['exit_date'])
        if ei is None: continue
        if xi is None: xi = n_days - 1
        xi = min(xi, n_days - 1)
        pnl_pct = t['pnl'] / 100.0
        tkr = t.get('ticker', ''); strategy = t.get('strategy', 'MOM')
        if strategy == 'PB':
            t_cap = initial_capital * PB_POSITION_SIZE
        elif tkr in TIER1_ASSETS: t_cap = initial_capital * 0.20
        elif tkr in TIER2_ASSETS: t_cap = initial_capital * 0.15
        else: t_cap = initial_capital * 0.10
    ...
    for d in range(1, n_days):
        ...
        for (ei, xi, pnl_pct, t_cap) in open_trades:
            if ei >= d: continue
            if xi < d - 1: mtm_total += t_cap * pnl_pct
            else:
    ...
    for (ei, xi, _, _t) in open_trades: open_arr[ei:xi+1] += 1
    ...
    return {
        ...
        'total_pct': round(total_pct, 1), 'max_dd': round(max_dd, 1),
        'sharpe': round(sharpe, 2), 'cagr': round(cagr, 1),
        'max_concurrent': max_concurrent, 'initial_capital': initial_capital,
        'pos_size_pct': position_size_pct, 'n_days': n_days,
    }
...
def get_today_signal(ticker, ind, market_regimes=None):
    i = ind['n'] - 1
    ...
    if market_regimes and is_inverse:
        ...
        regime = market_regimes.get(date_str, 'BULL')
        if regime not in ('BEAR', 'CAUTION'):
            return {'ticker': ticker, 'name': UNIVERSE.get(ticker, ticker),
                    'signal': 'NONE', 'price': round(price, 4),
                    'entry_price': None, 'stop_loss': None, 'take_profit': None,
                    'sl_pct': None, 'tp_pct': None, 'rr_ratio': None,
                    'trend': False, 'compressed': False, 'breakout': False,
                    'panic': False, 'conditions': 0,
                    'trend_detail': {}, 'comp_detail': {}, 'bo_detail': {},
                    'price_hist': [], 'ema50': 0, 'vol_pct_now': None, 'vol_rank_now': None}

    panic = check_panic(ind, i) if not is_inverse else False
    trend_ok, trend_d = check_trend(ind, i, ticker)
    comp_ok, comp_d = check_compression(ind, i)
    bo_ok, bo_d = check_breakout(ind, i)
    ...
    if not panic and trend_ok and comp_ok and bo_ok:
        entry_p, sl_p, tp_p, rr = compute_levels(ind, i, comp_d, ticker)
        if entry_p and rr >= 1.5: signal_type = 'FIRE'
        else: signal_type = 'WEAK'
    elif not panic and trend_ok and comp_ok and not bo_ok: signal_type = 'WATCH'
    elif not panic and trend_ok and not comp_ok: signal_type = 'TREND'
    elif panic: signal_type = 'PANIC'
    else: signal_type = 'NONE'

    entry_p = sl_p = tp_p = rr = sl_pct = tp_pct = None
    if signal_type == 'FIRE':
    ...
    hist_start = max(0, ind['n'] - 90); price_hist = []
    ...
    return {
        ...
        'signal': signal_type, 'price': round(price, 4),
        ...
        'trend': trend_ok, 'compressed': comp_ok, 'breakout': bo_ok,
        'panic': panic, 'conditions': conditions_met,
        'trend_detail': trend_d, 'comp_detail': comp_d, 'bo_detail': bo_d,
...
<header>
  <div class="brand">
    ...
    <div>
      <div class="brand-name">Momentum Engine v2.0</div>
        ...
        <tbody>${oos_trades.map(t=>{
          ...
          return`<tr>
            ...
            <td><span style="font-family:var(--mono);font-size:.58rem;padding:.1rem .3rem;border-radius:4px;${
              t.strategy==='PB'||t.strategy==='PB'||strategy==='MR'?'background:rgba(99,102,241,.15);color:#818cf8':'background:rgba(6,214,160,.1);color:var(--safe)'
...
def main():
    ...
    print(f"{DIM}  Momentum + Pullback + Régimen Bajista — trailing Chandelier{RST}")
    print(f"{BOLD}{'═'*65}{RST}\n")

    # ── Detectar régimen de mercado ──────────────────────────────────
    print(f"  {C}↓ Detectando régimen de mercado (SPY)...{RST}")
    regime_data = detect_market_regime()
    regime_now = regime_data.get('regime', 'NEUTRAL')
    market_regimes = regime_data.get('dates', {})
    regime_colors = {'BULL': G, 'CAUTION': Y, 'BEAR': R, 'NEUTRAL': DIM}
    print(f"  {regime_colors.get(regime_now, DIM)}{BOLD}→ Régimen actual: {regime_now}{RST}\n")
    ...
    for ticker, name in UNIVERSE.items():
        ...
        # Momentum backtest
        trades_is, diag_is   = backtest(ticker, ind_is, market_regimes)
        oos_start_str = OOS_START.strftime('%Y-%m-%d')
        trades_full, diag_full = backtest(ticker, ind_full, market_regimes)
        trades_oos = [t for t in trades_full if t['entry_date'] >= oos_start_str]
        ...
        # ── PULLBACK: estrategia complementaria REACTIVADA ───────────
        mom_open = build_momentum_open_set(trades_full)
        pb_full, pb_diag = backtest_pb(ticker, ind_full, mom_open)
        pb_oos = [t for t in pb_full if t['entry_date'] >= oos_start_str]
        pb_is_full, _ = backtest_pb(ticker, ind_is, build_momentum_open_set(trades_is))

        for t in pb_is_full: t['period'] = 'IS'
        for t in pb_oos:     t['period'] = 'OOS'

        all_is_pb.extend(pb_is_full)
        all_oos_pb.extend(pb_oos)
        ...
        print(f"MOM:{n_oos:>3}t {col}WR={wr_oos:.0f}%{RST} "
              f"PF={m_oos['pf'] if m_oos else 0:.2f}", end="")
        if n_pb > 0:
            m_pb = compute_metrics(pb_oos)
            pb_wr = m_pb['wr'] if m_pb else 0
            print(f" │ PB:{n_pb:>2}t WR={pb_wr:.0f}%", end="")
        print()
        ...
        # ── Señales de hoy ───────────────────────────────────────────
        sig = get_today_signal(ticker, ind, market_regimes)
        if sig:
            all_signals[ticker] = sig
    ...
    m_is_global  = compute_metrics(all_is_trades)
    m_oos_global = compute_metrics(all_oos_trades)
    m_pb_oos     = compute_metrics(all_oos_pb)
    all_is_combined  = all_is_trades  + all_is_pb
    all_oos_combined = all_oos_trades + all_oos_pb
    m_is_combined    = compute_metrics(all_is_combined)
    m_oos_combined   = compute_metrics(all_oos_combined)
    ...
    sigs_per_month = 0
    if oos_dates:
        first = datetime.strptime(oos_dates[0],  "%Y-%m-%d")
        last  = datetime.strptime(oos_dates[-1], "%Y-%m-%d")
        months = max((last - first).days / 30, 1)
        sigs_per_month = round(len(all_oos_combined) / months, 1)
    ...
    try:
        ...
        if port_start and port_end:
            ...
            if len(spy) >= 2:
                ...
                spy_equity = [round(float(v * 100), 2) for v in spy_mult]
                benchmark = {
                    'total_pct': round(spy_ret, 1), 'max_dd': round(spy_dd, 1),
                    'daily_dates': spy_dates, 'equity_curve': spy_equity,
                }
    except Exception:
        pass
    ...
    print(f"  {DIM}── Momentum + Pullback (combinado) ─────────────────{RST}")
    _row("Trades",           m_is_combined, m_oos_combined, 'n',         "{:.0f}")
    _row("Win Rate",         m_is_combined, m_oos_combined, 'wr',        "{:.1f}", "%")
    _row("Profit Factor",    m_is_combined, m_oos_combined, 'pf',        "{:.2f}")
    _row("Sharpe",           m_is_combined, m_oos_combined, 'sharpe',    "{:.2f}")
    _row("Max Drawdown",     m_is_combined, m_oos_combined, 'max_dd',    "{:.1f}", "%")
    _row("Expectativa/trade",m_is_combined, m_oos_combined, 'expectancy',"{:.3f}", "%")
    _row("Eficiencia pico",  m_is_combined, m_oos_combined, 'avg_efficiency', "{:.1f}", "%")
    _row("Total P&L",        m_is_combined, m_oos_combined, 'total_pct', "{:.1f}", "%")
    print(f"  {DIM}── Solo Momentum ───────────────────────────────────{RST}")
    _row("  Trades MOM",     m_is_global,   m_oos_global,   'n',   "{:.0f}")
    _row("  WR MOM",         m_is_global,   m_oos_global,   'wr',  "{:.1f}", "%")
    _row("  PF MOM",         m_is_global,   m_oos_global,   'pf',  "{:.2f}")
    if m_pb_oos:
        print(f"  {DIM}── Solo Pullback (OOS) ──────────────────────────────{RST}")
        print(f"  {'  Trades PB':<22} {'---':>10} {m_pb_oos['n']:>10.0f}")
        print(f"  {'  WR PB':<22} {'---':>10} {m_pb_oos['wr']:>10.1f}%")
        print(f"  {'  PF PB':<22} {'---':>10} {m_pb_oos['pf']:>10.2f}")
    ...
    dashboard_data = _san({
        ...
        "signals":       all_signals,
        "oos_trades":    all_oos_combined,
        "oos_trades_mr": all_oos_pb,
        "mr_metrics_oos": m_pb_oos,
        ...
        "params": {
            ...
            "TP_R_MULTIPLE":           TP_R_MULTIPLE,
            "MAX_HOLD_DAYS":           MAX_HOLD_DAYS,
            "CHANDELIER_ATR_PHASES":   f"{CHAN_ATR_1}/{CHAN_ATR_2}/{CHAN_ATR_3}",
            "CHANDELIER_FLOORS":       f"{CHAN_FLOOR_1}/{CHAN_FLOOR_2}/{CHAN_FLOOR_3}",
        },
    })
